...
import { embedNode as EmbeddedPlayerEmbedNode, preprocessHtml } from '../components/widgets/EmbeddedPlayers';
...
function linkify(content, mutate, hashtags, usertags, images, links) {
    ...
    content = content.replace(
        ...
        (match, preceeding1, preceeding2, user) => {
            ...
            return valid ? `${preceedings}<a href="/u?user=${userLower}">@${user}</a>` : `${preceedings}@${user}`;
        }
    );
